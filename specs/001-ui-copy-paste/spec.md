...
**Feature Branch**: `feat/001-ui-copy-paste`
**Created**: 2026-02-17
**Status**: Draft
...
7. **Given** a clipboard with modules, **When** user pastes, **Then** the new modules appear at a slight offset (+2 grid units) from the original position.
...
2. **Given** a cut module in clipboard, **When** user Pastes for the first time, **Then** the module appears as a new instance, preserving the same name/config as the cut module.
3. **Given** a cut module in clipboard and the user Pasted already once, **When** user Pastes again, **Then** the same logic of Copy/Paste is applied with smart incremental naming.
...
3. **Given** a selection, **When** user Shift-clicks a selected module, **Then** it is removed from selection.
4. **Given** any state, **When** user drags with **Left Mouse Button** on background, **Then** a selection rectangle appears; on release, enclosed modules are selected.
5. **Given** any state, **When** user drags with **Right Mouse Button** on background, **Then** the canvas pans (moves).
6. **Given** multiple modules selected, **When** user drags one, **Then** all selected modules move by the same delta.
7. **Given** multiple modules, **When** user clicks multiple modules, **Then** the properties of all modules are shown in the right bar (grouped per module).
8. **Given** any state, **When** user clicks the **Zoom In (+)** button, **Then** the canvas scales up (zooms in).
9. **Given** any state, **When** user clicks the **Zoom Out (-)** button, **Then** the canvas scales down (zooms out).
...
* **FR-002**: System MUST support additive Shift-click: clicking an unselected module adds it to selection; clicking a selected module removes it.
...
* **FR-009**: Pasted modules MUST have unique names generated by Smart Increment: if name ends in " (n)", increment n and repeat increment until the resulting name doesn't already exist in the config model; otherwise append " (1)". This also applies to each individual module in a group paste of multiple modules. 
* **FR-010**: Pasted modules MUST be positioned at a fixed offset of +2 grid units in X and Y from the original source position.
